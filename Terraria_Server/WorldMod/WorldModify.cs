...
namespace Terraria_Server.WorldMod
{
	...
	public static class WorldModify
	{
			...
			if (flag && TileRefs(i, j).Active)
			{
				...
				switch (type)
				{
					...
					case 10:
						...
						break;
					case 4:
						var tile = TileRefs(i, j);
						if (tile.FrameX < 66)
						{
							frameX = (short)(tile.FrameX + 66);
							TileRefs(i, j).SetFrameX(frameX);
						}
						else
						{
							frameX = (short)(tile.FrameX - 66);
					...
					case 42:
						...
						frameX = (short)(TileRefs(i, num2).FrameX + num3);
						TileRefs(i, num2).SetFrameX(frameX);

						frameX = (short)(TileRefs(i, num2 + 1).FrameX + num3);
						TileRefs(i, num2 + 1).SetFrameX(frameX);
						...
						break;
					case 93:
						int num4 = j - (int)(TileRefs(i, j).FrameX / 18);
						short num5 = 18;

						if (TileRefs(i, j).FrameX > 0)
							num5 = -18;

						frameX = (short)(TileRefs(i, num4).FrameX + num5);
						TileRefs(i, num4).SetFrameX(frameX);

						frameX = (short)(TileRefs(i, num4 + 1).FrameX + num5);
						TileRefs(i, num4 + 1).SetFrameX(frameX);

						frameX = (short)(TileRefs(i, num4 + 2).FrameX + num5);
						TileRefs(i, num4 + 2).SetFrameX(frameX);
					...
					case 33:
						...
						frameX = (short)(TileRefs(i, j).FrameX + num12);
						TileRefs(i, j).SetFrameX(frameX);
					...
					case 92:
						...
						frameX = (short)(TileRefs(i, num13).FrameX + num14);
						TileRefs(i, num13).SetFrameX(frameX);

						frameX = (short)(TileRefs(i, num13 + 1).FrameX + num14);
						TileRefs(i, num13 + 1).SetFrameX(frameX);

						frameX = (short)(TileRefs(i, num13 + 2).FrameX + num14);
						TileRefs(i, num13 + 2).SetFrameX(frameX);

						frameX = (short)(TileRefs(i, num13 + 3).FrameX + num14);
						TileRefs(i, num13 + 3).SetFrameX(frameX);

						frameX = (short)(TileRefs(i, num13 + 4).FrameX + num14);
						TileRefs(i, num13 + 4).SetFrameX(frameX);

						frameX = (short)(TileRefs(i, num13 + 5).FrameX + num14);
						TileRefs(i, num13 + 5).SetFrameX(frameX);
					...
					case 105:
						{
							...
							int num25 = (int)(TileRefs(i, j).FrameY / 18);
							int num26 = 0;
							while (num25 >= 2)
							{
								num25 -= 2;
							...
							if (type == 126 || type == 100 || type == 95)
							{
								...
								frameX = (short)(TileRefs(num7, num6).FrameX + num8);
								TileRefs(num7, num6).SetFrameX(frameX);

								frameX = (short)(TileRefs(num7, num6 + 1).FrameX + num8);
								TileRefs(num7, num6 + 1).SetFrameX(frameX);

								frameX = (short)(TileRefs(num7 + 1, num6).FrameX + num8);
								TileRefs(num7 + 1, num6).SetFrameX(frameX);

								frameX = (short)(TileRefs(num7 + 1, num6 + 1).FrameX + num8);
								TileRefs(num7 + 1, num6 + 1).SetFrameX(frameX);
							...
							else if (type == 34 || type == 35 || type == 36)
							{
								...
								for (int m = num10; m < num10 + 3; m++)
								{
									...
									{
										frameX = (short)(TileRefs(m, n).FrameX + num11);
										TileRefs(m, n).SetFrameX(frameX);
										noWire(m, n);
									}
								}
		...
		public static bool PlaceTile(Func<Int32, Int32, ITile> TileRefs, int i, int j, int type, bool mute = false, bool forced = false, int plr = -1, int style = 0)
		{
			...
			{
				if (forced || Collision.EmptyTile(i, j, false) || !Main.tileSolid[type] || (type == 23 && TileRefs(i, j).Type == 0 && TileRefs(i, j).Active) || (type == 2 && TileRefs(i, j).Type == 0 && TileRefs(i, j).Active) || (type == 60 && TileRefs(i, j).Type == 59 && TileRefs(i, j).Active) || (type == 70 && TileRefs(i, j).Type == 59 && TileRefs(i, j).Active))
				{
					if (type == 23 && (TileRefs(i, j).Type != 0 || !TileRefs(i, j).Active))
					{
						return false;
					}
					if (type == 2 && (TileRefs(i, j).Type != 0 || !TileRefs(i, j).Active))
					{
						return false;
					}
					if (type == 60 && (TileRefs(i, j).Type != 59 || !TileRefs(i, j).Active))
					{
						return false;
					}
					if (type == 81)
					...
					if (TileRefs(i, j).Liquid > 0 && (type == 3 || type == 4 || type == 20 || type == 24 || type == 27 || type == 32 || type == 51 || type == 69 || type == 72))
					{
						return false;
					}
					TileRefs(i, j).SetFrameY(0);
					...
					{
						if ((TileRefs(i - 1, j).Active && (Main.tileSolid[(int)TileRefs(i - 1, j).Type] ||
							TileRefs(i - 1, j).Type == 124 || (TileRefs(i - 1, j).Type == 5 && TileRefs(i - 1, j - 1).Type == 5 && TileRefs(i - 1, j + 1).Type == 5))) ||
							(TileRefs(i + 1, j).Active && (Main.tileSolid[(int)TileRefs(i + 1, j).Type] || TileRefs(i + 1, j).Type == 124 || (
							TileRefs(i + 1, j).Type == 5 && TileRefs(i + 1, j - 1).Type == 5 && TileRefs(i + 1, j + 1).Type == 5))) ||
							(TileRefs(i, j + 1).Active && Main.tileSolid[(int)TileRefs(i, j + 1).Type]))
						{
							TileRefs(i, j).SetActive(true);
							TileRefs(i, j).SetType((byte)type);
							SquareTileFrame(TileRefs, i, j);
		...
		public static void KillTile(Func<Int32, Int32, ITile> TileRefs, int i, int j, bool fail = false, bool effectOnly = false, bool noItem = false)
		{
			...
			if (i >= 0 && j >= 0 && i < Main.maxTilesX && j < Main.maxTilesY)
			{
				var tileType = TileRefs(i, j).Type;

				if (TileRefs(i, j).Active)
				{
					if (j >= 1 && TileRefs(i, j - 1).Active && ((TileRefs(i, j - 1).Type == 5 && tileType != 5) ||
						(TileRefs(i, j - 1).Type == 21 && tileType != 21) || (TileRefs(i, j - 1).Type == 26 && tileType != 26) ||
							(TileRefs(i, j - 1).Type == 72 && tileType != 72) || (TileRefs(i, j - 1).Type == 12 && tileType != 12)) &&
								(TileRefs(i, j - 1).Type != 5 || ((TileRefs(i, j - 1).FrameX != 66 || TileRefs(i, j - 1).FrameY < 0 || TileRefs(i, j - 1).FrameY > 44) &&
									(TileRefs(i, j - 1).FrameX != 88 || TileRefs(i, j - 1).FrameY < 66 || TileRefs(i, j - 1).FrameY > 110) && TileRefs(i, j - 1).FrameY < 198)))
					{
						return;
					}
					if (!effectOnly && !WorldModify.stopDrops)
					{
						if (tileType == 3 || tileType == 110)
						{
					...
					for (int m = 0; m < num; m++)
					{
						...
						if (tileType == 1 || tileType == 16 || tileType == 17 || tileType == 38 || tileType == 39 || tileType == 41 || tileType == 43 || tileType == 44 || tileType == 48 || Main.tileStone[(int)tileType] || tileType == 85 || tileType == 90 || tileType == 92 || tileType == 96 || tileType == 97 || tileType == 99 || tileType == 105 || tileType == 117 || tileType == 130 || tileType == 131 || tileType == 132 || tileType == 135 || tileType == 135 || tileType == 137 || tileType == 142 || tileType == 143 || tileType == 144)
						{
							num5 = 1;
						}
						if (tileType == 33 || tileType == 95 || tileType == 98 || tileType == 100)
						{
							num5 = 6;
						}
						if (tileType == 5 || tileType == 10 || tileType == 11 || tileType == 14 || tileType == 15 || tileType == 19 || tileType == 30 || tileType == 86 || tileType == 87 || tileType == 88 || tileType == 89 || tileType == 93 || tileType == 94 || tileType == 104 || tileType == 106 || tileType == 114 || tileType == 124 || tileType == 128 || tileType == 139)
						{
							num5 = 7;
						}
						if (tileType == 21)
						...
						if (tileType == 6 || tileType == 26)
						{
							num5 = 8;
						}
						if (tileType == 7 || tileType == 34 || tileType == 47)
						{
							num5 = 9;
						}
						if (tileType == 8 || tileType == 36 || tileType == 45 || tileType == 102)
						{
							num5 = 10;
						}
						if (tileType == 9 || tileType == 35 || tileType == 42 || tileType == 46 || tileType == 126 || tileType == 136)
						{
							num5 = 11;
						}
						if (tileType == 12)
						...
						if (tileType == 53 || tileType == 81)
						{
							num5 = 32;
						}
						if (tileType == 56 || tileType == 75)
						{
							num5 = 37;
						}
						if (tileType == 57 || tileType == 119 || tileType == 141)
						{
							num5 = 36;
						}
						...
						if (tileType == 61 || tileType == 62 || tileType == 74 || tileType == 80)
						{
							num5 = 40;
						}
						if (tileType == 69)
						...
						if (tileType == 107 || tileType == 121)
						{
							num5 = 48;
						}
						if (tileType == 108 || tileType == 122 || tileType == 134 || tileType == 146)
						{
							num5 = 49;
						}
						if (tileType == 111 || tileType == 133 || tileType == 145)
						{
							num5 = 50;
						}
						if (tileType == 149)
						...
						if (tileType == 58 || tileType == 76 || tileType == 77)
						{
							if (WorldModify.genRand.Next(2) == 0)
							{
								num5 = 6;
						...
						if (tileType == 129)
						{
							...
							else if (TileRefs(i, j).FrameX == 18 || TileRefs(i, j).FrameX == 72 || TileRefs(i, j).FrameX == 126)
							{
								num5 = 69;
							}
							else
						...
						if ((tileType == 34 || tileType == 35 || tileType == 36 || tileType == 42) && Main.rand.Next(2) == 0)
						{
							num5 = 6;
						}
					}
					...
					if (fail)
					{
						if (tileType == 2 || tileType == 23 || tileType == 109)
						{
					...
					{
						int num8 = 0;
						if (tileType == 0 || tileType == 2 || tileType == 109)
						{
							num8 = 2;
						}
						else if (tileType == 1)
						...
						else if (tileType == 3 || tileType == 73)
						{
						...
						else if (tileType == 5)
						{
							if (TileRefs(i, j).FrameX >= 22 && TileRefs(i, j).FrameY >= 198)
							{
								...
								{
									int num10 = j;
									while ((!TileRefs(i, num10).Active || !Main.tileSolid[(int)TileRefs(i, num10).Type] ||
										Main.tileSolidTop[(int)TileRefs(i, num10).Type]))
									{
										num10++;
									}
						...
						else if (tileType == 149)
						{
							if (TileRefs(i, j).FrameX == 0 || TileRefs(i, j).FrameX == 54)
							{
								num8 = 596;
							}
							else if (TileRefs(i, j).FrameX == 18 || TileRefs(i, j).FrameX == 72)
							{
								num8 = 597;
							}
							else if (TileRefs(i, j).FrameX == 36 || TileRefs(i, j).FrameX == 90)
							{
								num8 = 598;
							}
						}
						else if (tileType == 13)
						{
							...
							else if (TileRefs(i, j).FrameX == 54)
							{
								num8 = 350;
							}
							else if (TileRefs(i, j).FrameX == 72)
							{
								num8 = 351;
							}
							else
						...
						else if (tileType == 116)
						{
							num8 = 408;
						}
						else if (tileType == 117)
						{
							num8 = 409;
						}
						...
						else if (tileType == 118)
						{
							num8 = 412;
						}
						else if (tileType == 119)
						{
							num8 = 413;
						}
						...
						else if (tileType == 136)
						{
							num8 = 538;
						}
						else if (tileType == 137)
						{
							num8 = 539;
						}
						else if (tileType == 141)
						{
							num8 = 580;
						}
						else if (tileType == 145)
						{
							num8 = 586;
						}
						else if (tileType == 146)
						{
							num8 = 591;
						}
						...
						else if (tileType == 148)
						{
							num8 = 594;
						}
						else if (tileType == 135)
						{
							if (TileRefs(i, j).FrameY == 0)
							{
								num8 = 529;
						...
						else if (tileType == 130)
						{
							num8 = 511;
						}
						else if (tileType == 131)
						{
							num8 = 512;
						}
						else if (tileType == 61 || tileType == 74)
						{
							if (TileRefs(i, j).FrameX == 144)
							{
								Item.NewItem(i * 16, j * 16, 16, 16, 331, WorldModify.genRand.Next(2, 4), false, 0);
							}
							else if (TileRefs(i, j).FrameX == 162)
							{
								num8 = 223;
							}
							else if (TileRefs(i, j).FrameX >= 108 && TileRefs(i, j).FrameX <= 126 && WorldModify.genRand.Next(100) == 0)
							{
								num8 = 208;
							}
						...
						else if (tileType == 59 || tileType == 60)
						{
							num8 = 176;
						}
						else if (tileType == 71 || tileType == 72)
						{
						...
						else if (tileType >= 63 && tileType <= 68)
						{
							num8 = (int)(tileType - 63 + 177);
						}
						else if (tileType == 50)
						{
						...
						else if (Main.tileAlch[(int)tileType] && tileType > 82)
						{
							...
							if (flag)
							{
								Item.NewItem(i * 16, j * 16, 16, 16, 307 + num11, WorldModify.genRand.Next(1, 4), false, 0);
							}
						}
					...
					if (tileType == 58 && j > Main.maxTilesY - 200)
					{
		...
		public static void UpdateWorld(Func<Int32, Int32, ITile> TileRefs, ISender Sender)
		{
			...
			while ((float)num3 < (float)(Main.maxTilesX * Main.maxTilesY) * num)
			{
				...
				if (true)
				{
					...
					{
						hardUpdateWorld(TileRefs, TileX, TileY);
						if (Tile.Type == 80)
						{
							if (genRand.Next(15) == 0)
							{
		...
		public static void PlaceWall(Func<Int32, Int32, ITile> TileRefs, int i, int j, int type, bool mute = false)
		{
			...
			if ((int)TileRefs(i, j).Wall != type)
			{
				for (int k = i - 1; k < i + 2; k++)
				{
					for (int l = j - 1; l < j + 2; l++)
					{
						if (TileRefs(k, l).Wall > 0 && (int)TileRefs(k, l).Wall != type)
						{
							bool flag = false;
							if (TileRefs(i, j).Wall == 0 && (type == 2 || type == 16) && (TileRefs(k, l).Wall == 2 || TileRefs(k, l).Wall == 16))
							{
								flag = true;
							}
							if (!flag)
		...
		public static void WallFrame(Func<Int32, Int32, ITile> TileRefs, int i, int j, bool resetFrame = false)
		{
			...
			if (i >= 0 && j >= 0 && i < Main.maxTilesX && j < Main.maxTilesY && TileRefs(i, j).Wall > 0)
			{
				...
				//byte arg_89_0 = TileRefs(i, j).FrameX;
				//byte arg_9B_0 = TileRefs(i, j).FrameY;
				Rectangle rectangle;
				rectangle.X = -1;
				rectangle.Y = -1;
				if (i - 1 < 0)
				...
				if (resetFrame)
				{
					...
					TileRefs(i, j).SetFrameNumber((byte)num9);
				}
				else
					num9 = (int)TileRefs(i, j).FrameNumber;
