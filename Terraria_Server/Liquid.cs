...
namespace Terraria_Server
{
	public struct Liquid //let's hope the change to struct doesn't break anything
	{
		...
		/*public static void NetAddWater(Func<Int32, Int32, ITile> TileRefs, int x, int y)
		{
			if (x >= Main.maxTilesX - 5 || y >= Main.maxTilesY - 5 || x < 5 || y < 5 || TileRefs(x, y).Liquid == 0)
				return;
			...
			for (int i = 0; i < Liquid.numLiquid; i++)
			{
				if (Main.liquid[i].x == x && Main.liquid[i].y == y)
				{
					...
					TileRefs(x, y).SetSkipLiquid(true);
					return;
				}
			}
