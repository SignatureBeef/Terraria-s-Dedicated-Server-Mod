...
namespace Terraria_Server
{
    ...
    public class Projectile : BaseEntity, ISender
    {
        ...
        public static int NewProjectile(float X, float Y, float SpeedX, float SpeedY, ProjectileType Type, int Damage, float KnockBack, int Owner = 255)
        {
            ...
            int num = ReserveSlot(Owner, Type); //, editor.ITileAt, sandbox);

            //if (Owner < 255 && Owner >= 0) sandbox.Apply(Main.players[Owner]);
            //else sandbox.Apply();
        ...
        public static int ReserveSlot(int owner, ProjectileType type) //s, Func<Int32, Int32, ITile> TileRefs, ISandbox sandbox)
        {
            lock (identityMap)
            {
                //if (Terraria_Server.Commands.Commands.Resetprjectiles) CheckReserves(type); //, TileRefs, sandbox);
        ...
        //public static void CheckReserves(ProjectileType type) //, Func<Int32, Int32, ITile> TileRefs, ISandbox sandbox)
        //{
        //    var items = (from x in Main.projectile where x.type == type && x.timeLeft <= 0 select x).ToArray();
        //    foreach (var proj in items) proj.Reset(); //s Kill(TileRefs, sandbox);
        //}
