...
namespace Terraria_Server
{
    public class Recipe
    {
        ...
        {
            Statics.recipe[numRecipes] = newRecipe;
        ...
        public void Create(World world)
        {
            for (int i = 0; i < maxRequirements; i++)
            {
                ...
                {
                    if (world.getPlayerList()[Statics.myPlayer].inventory[j].IsTheSameAs(this.requiredItem[i]))
                    {
                        if (world.getPlayerList()[Statics.myPlayer].inventory[j].stack > stack)
                        {
                            Item item1 = world.getPlayerList()[Statics.myPlayer].inventory[j];
                            item1.stack -= stack;
                            stack = 0;
                        }
                        else
                        {
                            stack -= world.getPlayerList()[Statics.myPlayer].inventory[j].stack;
                            world.getPlayerList()[Statics.myPlayer].inventory[j] = new Item();
                        }
                    }
                    if (stack <= 0)
                    {
        ...
        public static void FindRecipes(World world)
        {
            int num = Statics.availableRecipe[Statics.focusRecipe];
            float num2 = Statics.availableRecipeY[Statics.focusRecipe];
            for (int i = 0; i < maxRecipes; i++)
            {
                Statics.availableRecipe[i] = 0;
            }
            Statics.numAvailableRecipes = 0;
            for (int j = 0; j < maxRecipes; j++)
            {
                if (Statics.recipe[j].createItem.type == 0)
                {
                    break;
                }
                bool flag = true;
                ...
                {
                    if (Statics.recipe[j].requiredItem[n].type == 0)
                    {
                        break;
                    }
                    int stack = Statics.recipe[j].requiredItem[n].stack;
                    for (int num7 = 0; num7 < 0x2c; num7++)
                    {
                        if (world.getPlayerList()[Statics.myPlayer].inventory[num7].IsTheSameAs(Statics.recipe[j].requiredItem[n]))
                        {
                            stack -= world.getPlayerList()[Statics.myPlayer].inventory[num7].stack;
                        }
                        if (stack <= 0)
                        {
                            break;
                ...
                if (flag)
                {
                    ...
                    {
                        if (Statics.recipe[j].requiredTile[num8] == -1)
                        {
                            break;
                        }
                        if (!world.getPlayerList()[Statics.myPlayer].adjTile[Statics.recipe[j].requiredTile[num8]])
                        {
                            flag2 = false;
                            break;
                        }
                    ...
                    if (flag2)
                    {
                        Statics.availableRecipe[Statics.numAvailableRecipes] = j;
                        Statics.numAvailableRecipes++;
                    }
                }
            }
            for (int k = 0; k < Statics.numAvailableRecipes; k++)
            {
                if (num == Statics.availableRecipe[k])
                {
                    Statics.focusRecipe = k;
                    break;
                }
            }
            if (Statics.focusRecipe >= Statics.numAvailableRecipes)
            {
                Statics.focusRecipe = Statics.numAvailableRecipes - 1;
            }
            if (Statics.focusRecipe < 0)
            {
                Statics.focusRecipe = 0;
            }
            float num10 = Statics.availableRecipeY[Statics.focusRecipe] - num2;
            for (int m = 0; m < maxRecipes; m++)
            {
                Statics.availableRecipeY[m] -= num10;
            }
        }
