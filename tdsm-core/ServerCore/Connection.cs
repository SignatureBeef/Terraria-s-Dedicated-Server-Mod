...
namespace tdsm.core.ServerCore
{
    public abstract class Connection
    {
                        ...
                        case Message.BYTES:
                            {
                                ...
                                //ProgramLog.Log ("{1}: Adding bytes {0}", data.Length, Thread.CurrentThread.IsThreadPoolThread);
                                break;
                            }
                        ...
                        case Message.SEGMENT:
                            {
                                ...
                                if (txc > 0 && txList[txc - 1].Array == txBuffer)
                                {
                                    ...
                                    //ProgramLog.Log ("{5}: Coalescing {0}, {1} and {2}, {3} [{4}]", seg.Offset, seg.Count, (txHead + txPrepared) % txBuffer.Length, len, nseg.Count, Thread.CurrentThread.IsThreadPoolThread);
                                ...
                                else
                                {
                                    ...
                                    //ProgramLog.Log ("{2}: Adding segment {0}, {1}", offset, len, Thread.CurrentThread.IsThreadPoolThread);
                        ...
                        default:
                            {
                                ...
                                //ProgramLog.Log ("{1}: Adding custom {0}", data.Count, Thread.CurrentThread.IsThreadPoolThread);
                                escape = true;
                                break;
                            }
        ...
        protected void TxListClear()
        {
            ...
            for (int i = 0; i < txList.Count; i++)
            {
                ...
                if (seg.Array == txBuffer)
                {
                    ...
                    txCount -= count;
                    txPrepared -= count;
                    if (txPrepared < 0 || txCount < 0) ProgramLog.Log("{0} {1}", txCount, txPrepared);
                }
            }
        ...
        protected void SendCompleted(SocketAsyncEventArgsExt argz)
        {
            ...
            {
                ProgramLog.Log("Exception in connection send callback: {0}", e);
            }

            try
            {
                //ProgramLog.Log ("SendCompleted [");
                ...
                //ProgramLog.Log ("} SendCompleted " + sending);
            }
            catch (Exception e)
            {
                ProgramLog.Log("Exception in connection send callback: {0}", e);
        ...
        protected void ReceiveCompleted(SocketAsyncEventArgsExt argz)
        {
            ...
            {
                ProgramLog.Log("Exception in connection receive callback: {0}", e);
            }
        }
        ...
        protected void KickCompleted(SocketAsyncEventArgsExt argz)
        {
            ...
            {
                ProgramLog.Log("Exception in connection disconnect callback: {0}", e);
            }
        }
        ...
        {
            //			ProgramLog.Log ("HandleError {0}", err);
...
#pragma warning restore 420
            ...
            {
                ProgramLog.Log("Exception while handling connection closure: {0}", e);
            }
        }
        ...
        class ArgsPool<T> : Stack<T> where T : SocketAsyncEventArgsExt, new()
        {
            ...
            public T Take(Connection conn)
            {
                ...
                lock (this)
                {
                    ...
                    {
                        total += 1;
                        ProgramLog.Log("ArgsPool<{0}> capacity is now: {1}.", typeof(T).Name, total);
                        args = new T();
                    }
                }
                args.conn = conn;
            ...
            public void Put(SocketAsyncEventArgsExt args)
            {
                ...
                if (!(args is T))
                {
                    ProgramLog.Log("ArgsPool type mismatch.");
                    return;
                }
                ...
                lock (this)
                {
                    if (args.conn == null)
                    {
                        ProgramLog.Log("{0} freed twice.", typeof(T).Name);
                        return;
                    }
