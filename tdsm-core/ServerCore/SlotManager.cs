...
namespace tdsm.core.ServerCore
{
    public static class SlotManager
    {
        ...
        static bool AssignSlot(ClientConnection conn, int id)
        {
            ...
            var slot = tdsm.api.Callbacks.NetplayCallback.slots[id] as ServerSlot;
        ...
        public static void FreeSlot(int id)
        {
            ...
            lock (syncRoot)
            {
                if (handovers[id] != null)
                {
                    ...
                    {
                        assignedTo = null;
                        ProgramLog.Log("Slot {0} handover failed.", id);
                    }
                    else
                        ProgramLog.Log("Slot {0} handover successful.", id);
                }
                ...
                {
                    PushSlot(id);
                    ProgramLog.Log("Freed slot {0}", id);
        ...
        internal static void QCommand(ISender sender, ArgumentList args)
        {
            ...
            lock (syncRoot)
            {
                ...
                for (int q = 3; q >= 0; q--)
                {
                    ...
                    if (queue.Count > 0)
                    {
                        sender.Message(255, Color.Snow, "Queued with priority {0}:", q);
                        ...
                        for (int i = 0; i < queue.count; i++)
                        {
                            ...
                            if (conn != null)
                            {
                                sender.Message(255, Color.Beige, "  {0}. {1} from {2}", c, conn.Player.name, conn.RemoteAddress);
                                c += 1;
                            }
                        }
                    }
