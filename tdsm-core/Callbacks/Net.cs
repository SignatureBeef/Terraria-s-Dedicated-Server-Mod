...
namespace tdsm.core.Callbacks
{
    public static class Net
    {
        ...
        public static void sendWater(int x, int y)
        {
            for (int i = 0; i < 256; i++)
            {
                if ((/*NewNetMessage.buffer[i].broadcast ||*/ Server.slots[i].state >= SlotState.SENDING_TILES) && Server.slots[i].Connected)
                {
                    int num = x / 200;
                    int num2 = y / 150;
                    if (Server.slots[i].tileSection[num, num2])
                    {
        ...
        public static void syncPlayers()
        {
            ...
            for (int i = 0; i < 255; i++)
            {
                ...
                if (Server.slots[i].state == SlotState.PLAYING)
                {
                    ...
                    {
                        string text = Server.slots[i].conn.RemoteAddress;
                        string a = text;
                    ...
                    if (!Server.slots[i].announced)
                    {
                        Server.slots[i].announced = true;
                ...
                else
                {
                    ...
                    if (Server.slots[i].announced)
                    {
                        Server.slots[i].announced = false;
                        NewNetMessage.SendData(25, -1, i, Server.slots[i].oldName + " " + Lang.mp[20], 255, 255f, 240f, 20f, 0);
                        if (Main.dedServ)
                        {
                            ProgramLog.Log(Server.slots[i].oldName + " " + Lang.mp[20]);
                        }
                    }
                }
            }
